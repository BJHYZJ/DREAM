...
from dream_fetch_footprint import swept_clear,navigation_swept_clear
...
class HeadingRouteExecutor:
    ...
    def clear(self,start,end):
        ...
        return navigation_swept_clear(self.occupancy,footprint,start,end)
    ...
    def translate(self,goal,planned_heading=None):
        ...
        for step in range(max(180,int(distance/.08*self.io.base.control_freq*2))):
            ...
            if stagnant>70:
                self.stop("translation_controller_stall")
                if not reverse:
                    footprint=self.footprint_provider()
                    bearing=math.atan2(delta[1],delta[0])
                    relative=bearing-pose[2]
                    direction=np.array([math.cos(relative),math.sin(relative)])
                    extent=float(np.max(np.asarray(footprint["vertices_base_xy"])@direction))
                    distance=extent+footprint["padding_m"]+3*self.occupancy.resolution
                    self.occupancy.infer_stall_obstacle(pose[:2],bearing,distance=distance)
                    self.event("heading_stall_obstacle_inferred",base=pose.tolist(),
                        attempted_bearing_rad=bearing,distance_m=distance,
                        evidence="commanded forward motion without measured progress")
                return False
