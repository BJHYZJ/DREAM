...
def main():
    ...
    parser.add_argument("--spectator-mode",choices=("overhead","tool-front-placement"),default="overhead",
                        help="Display-only framing during exact replay; never changes robot controls")
    parser.add_argument("--placement-diagnostic",action="store_true",
                        help="After exact full replay, test local placement recovery with partial observed memory; NEVER a full-task result")
    parser.add_argument("--diagnostic-step-budget",type=int,default=2400)
    parser.add_argument("--dump-robot-footprint",action="store_true",
                        help="Read-only final robot collision footprint; not a navigation success")
    parser.add_argument("--heading-diagnostic",type=Path,
                        help="After exact prefix, physically test this offline observed-map route; NOT a full task")
    parser.add_argument("--place-after-heading-diagnostic",action="store_true",
                        help="After a verified local navigation, test actual learned-observation placement; still NOT a full task")
    args=parser.parse_args()
    if args.diagnostic_step_budget<=0:
        parser.error("Diagnostic step budget must be positive")
    if args.placement_diagnostic and args.render_review:
        parser.error("Keep alternate review rendering and controller diagnostics in separate outputs")
    if args.spectator_mode!="overhead" and not args.render_review:
        parser.error("--spectator-mode requires --render-review")
    if args.heading_diagnostic and (args.placement_diagnostic or args.render_review):
        parser.error("Keep heading diagnostics, placement diagnostics and spectator rendering in separate outputs")
    if args.place_after_heading_diagnostic and not args.heading_diagnostic:
        parser.error("--place-after-heading-diagnostic requires --heading-diagnostic")
    ...
    try:
        ...
        if args.render_review:
            ...
            video=InstructionReplayVideo(source,args.render_review,args.output,mode=args.spectator_mode)
        ...
        motion_steps = {row["step"] for row in controls if "arm_joint_motion" in row}
        motion_errors = []
        motion_matches = True
        motion = JointMotionSampler(io, control_step=lambda: step,
                                    selected_steps=motion_steps) if motion_steps else None
        with motion if motion is not None else nullcontext():
            for control,truth in zip(controls,original_rows):
                ...
                if step in motion_steps:
                    actual_motion = motion.records.get(step, {})
                    recorded_motion = control["arm_joint_motion"]
                    matched = actual_motion.keys() == recorded_motion.keys()
                    largest_error = 0.
                    for key in ("physics_dt_s", "initial_joint_positions_rad", "joint_positions_rad"):
                        actual_value = np.asarray(actual_motion.get(key, []), dtype=float)
                        recorded_value = np.asarray(recorded_motion.get(key, []), dtype=float)
                        if (actual_value.shape != recorded_value.shape or actual_value.size == 0
                                or not np.isfinite(actual_value).all() or not np.isfinite(recorded_value).all()):
                            matched = False
                        else:
                            error = float(np.max(np.abs(actual_value - recorded_value)))
                            largest_error = max(largest_error, error)
                            matched &= error <= 1e-7
                    motion_matches &= matched
                    motion_errors.append(dict(step=step, matched=bool(matched), maximum_error=largest_error))
        ...
        report=dict(source_run=str(source.resolve()),steps=len(rows),maximum_error=maxima,
            tolerance=args.tolerance,physical_reexecution_passed=motion_matches and all(v<=args.tolerance for v in maxima.values()),
            arm_joint_motion_reexecution=dict(passed=bool(motion_matches),
                control_steps=sorted(motion_steps), tolerance_rad=1e-7,
                maximum_error=max((row["maximum_error"] for row in motion_errors), default=0.)),
        ...
        if motion is not None:
            (args.output/"arm_joint_motion.json").write_text(json.dumps(motion.records)+"\n")
            (args.output/"arm_joint_motion_errors.json").write_text(json.dumps(motion_errors)+"\n")
        (args.output/"audit.json").write_text(json.dumps(report,indent=2)+"\n")
        print(json.dumps({k:v for k,v in report.items() if k!="contact_audit"},indent=2),flush=True)
        if args.dump_robot_footprint:
            from dream_fetch_footprint import capture_robot_footprint
            held_geometry=[e for e in events if e["event"]=="measured_carry_envelope"]
            radius=(held_geometry[-1]["payload_radius_from_depth_m"]
                    if held_geometry and truth["task_stage"]=="placement_search" else None)
            grasp_geometry=[e for e in events if e["event"]=="depth_grasp_center"]
            height=grasp_geometry[-1].get("height_m") if radius and grasp_geometry else None
            footprint=capture_robot_footprint(io,payload_radius=radius,payload_height=height)
            footprint.update(source_run=str(source.resolve()),source_event="measured_carry_envelope" if radius else None,
                physical_reexecution_passed=report["physical_reexecution_passed"],
                complete_controls_replayed=len(rows)==len(controls),navigation_execution_performed=False)
            (args.output/"robot_footprint.json").write_text(json.dumps(footprint,indent=2)+"\n")
        if args.placement_diagnostic:
            from diagnose_instruction_placement import run_diagnostic
            run_diagnostic(io,source,args.output/"placement_diagnostic",report,instruction,
                           target,destination,pickup,receptacle,step_budget=args.diagnostic_step_budget)
        if args.heading_diagnostic:
            from diagnose_heading_execution import run_diagnostic
            navigation=run_diagnostic(io,source,args.output/"heading_diagnostic",report,args.heading_diagnostic,
                                     target,destination,cart,step_budget=args.diagnostic_step_budget)
            if args.place_after_heading_diagnostic:
                if not navigation["navigation_diagnostic_passed"]:
                    raise RuntimeError("Navigation diagnostic failed; do not start placement from an unverified endpoint")
                from diagnose_instruction_placement import run_diagnostic as place_diagnostic
                place_diagnostic(io,source,args.output/"placement_diagnostic",report,instruction,
                                 target,destination,pickup,receptacle,step_budget=args.diagnostic_step_budget,
                                 navigation_record=args.output/"heading_diagnostic")
    finally:
