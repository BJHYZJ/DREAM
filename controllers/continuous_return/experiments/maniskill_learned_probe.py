#!/usr/bin/env python3
"""Fetch control, camera observations, and learned-perception diagnostics.

SimulatorIO supports the instruction-based mobile manipulation runner.
The standalone entry point performs a physical observation scan.
"""
...
os.environ.setdefault("VK_ICD_FILENAMES", next((str(p) for p in (
    ...
    Path("/usr/share/vulkan/icd.d/lvp_icd.x86_64.json")) if p.is_file()),
...
def configure_torso_drives(agent):
    """Configure both control modes before the first recorded or replayed step."""
    original_mode = agent.control_mode
    for mode in ("pd_joint_pos", "pd_ee_delta_pose"):
        agent.set_control_mode(mode)
        body = agent.controller.controllers["body"]
        if body.config.joint_names[2] != "torso_lift_joint":
            raise ValueError("Unexpected Fetch torso joint ordering")
        stiffness = np.broadcast_to(body.config.stiffness, 3).copy()
        damping = np.broadcast_to(body.config.damping, 3).copy()
        stiffness[2], damping[2] = 4000.0, 400.0
        body.config.stiffness = stiffness.tolist()
        body.config.damping = damping.tolist()
        # Retain the original force limits and all other actuator settings.
        body.set_drive_property()
    agent.set_control_mode(original_mode)
    agent.controller.set_drive_property()
...
class SimulatorIO:
    ...
    def __init__(self, env):
        ...
        self._torso_reference = float(q[3])
        self._torso_reference_velocity = 0.0
        self._torso_dt = 1.0 / float(self.base.control_freq)
        self.grip = 1.
    ...
    def smooth_body_target(self):
        """Continuous torso reference shared by joint and Cartesian control."""
        dt = self._torso_dt
        limits = array(self.robot.robot.get_qlimits())[0, 3]
        goal = float(np.clip(self.body[2], limits[0] + .01, limits[1] - .01))
        error = goal - self._torso_reference
        # Critically damped reference dynamics, bounded in SI units.
        acceleration = float(np.clip(16.0 * error - 8.0 * self._torso_reference_velocity, -.10, .10))
        velocity = float(np.clip(self._torso_reference_velocity + acceleration * dt, -.04, .04))
    ...
    def torso_trace(self):
        return dict(arm_joint_positions_rad=array(self.robot.robot.get_qpos())[0,[5,7,8,9,10,11,12]].tolist(),
                    arm_joint_velocities_rad_s=array(self.robot.robot.get_qvel())[0,[5,7,8,9,10,11,12]].tolist(),
                    torso_position_m=float(array(self.robot.robot.get_qpos())[0, 3]),
    ...
    def settle_torso(self, max_steps=260, check=None, hold_tcp=False):
        ...
        if hold_tcp:
            ...
            if original_mode != "pd_ee_delta_pose":
                self.robot.set_control_mode("pd_ee_delta_pose")
                self.robot.controller.reset()
        try:
            for step in range(max_steps):
                if check is not None and step % 10 == 0:
                    check()
                if hold_tcp:
                    current = array(self.robot.tcp_pose.to_transformation_matrix())[0]
                    root = array(self.robot.torso_lift_link.pose.to_transformation_matrix())[0, :3, :3]
                    error = root.T @ (target_pose[:3, 3] - current[:3, 3])
                    rotation = root.T @ Rotation.from_matrix(target_pose[:3, :3] @ current[:3, :3].T).as_rotvec()
                    scale = float(self.robot.controller.controllers["arm"].config.rot_lower)
                    self.command_ee(np.r_[np.clip(error * 3., -.08, .08), np.clip(.2 * rotation / scale, -.12, .12)])
                    current = array(self.robot.tcp_pose.to_transformation_matrix())[0]
                    pose_settled = (np.linalg.norm(target_pose[:3, 3] - current[:3, 3]) < .015
                        and np.linalg.norm(Rotation.from_matrix(target_pose[:3, :3] @ current[:3, :3].T).as_rotvec()) < .06)
                else:
                    self.command()
                    pose_settled = True
                measured = self.torso_trace()
                ...
                if (step >= 19 and abs(self._torso_reference - goal) < .001
                        ...
                        and abs(measured['torso_velocity_m_s']) < .015 and pose_settled):
                    if check is not None:
                        check()
                    return
            raise RuntimeError("Torso and payload pose did not settle within the motion budget")
        finally:
            if hold_tcp:
    ...
    def command(self, forward=0., yaw_rate=0., grip=None):
        ...
        action[:7] = self.arm
        action[7] = self.grip
        action[8:11] = np.clip((self.smooth_body_target() - q[[4, 6, 3]]) / .1, -1., 1.)
        ...
        self.trace.append(dict(step=self.step_id, base_xyyaw=self.pose().tolist(), action=action.tolist(),
            control_mode=self.robot.control_mode,tcp_xyz=array(self.robot.tcp_pose.p)[0].tolist(), **self.torso_trace()))
    ...
    def command_ee(self, delta=None, grip=None, forward=0.,yaw_rate=0.):
        ...
        if delta is not None:
            action[:6] = delta
        action[6] = self.grip
        action[7:10] = np.clip((self.smooth_body_target()-q[[4,6,3]])/.1,-1.,1.)
    ...
    def fold_default_arm(self, check=None, *, payload_radius=0.0, payload_height=0.0, withdraw=False, minimum_tcp_height=None):
        """Execute and verify each intermediate arm-return posture."""
        from instruction_arm_return import confirm_initial_stow, execute_return
        if (check is None and not withdraw and payload_radius == 0.0
                and payload_height == 0.0 and minimum_tcp_height is None):
            initial = confirm_initial_stow(self)
            if initial:
                return initial
        return execute_return(self, check=check, payload_radius=payload_radius, payload_height=payload_height, withdraw=withdraw, minimum_tcp_height=minimum_tcp_height)

    def prepare_scan(self):
        return self.fold_default_arm()
