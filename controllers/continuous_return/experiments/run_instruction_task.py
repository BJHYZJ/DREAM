...
from instruction_execution_clock import RobotActionBudget
...
def main():
    ...
    parser.add_argument("--simulation-time-limit-seconds",type=float,
        help="Robot action duration; excludes loading, inference waiting and output saving")
    parser.add_argument("--video",action="store_true")
    parser.add_argument("--heading-navigation",action="store_true",
        help="Opt-in measured-footprint DREAM A* fallback; development until independently audited")
    args=parser.parse_args()
    if args.simulation_time_limit_seconds is not None and not 0<args.simulation_time_limit_seconds<=1800:
        parser.error("Robot action time limit must be positive and at most 1800 seconds")
    ...
    (args.output/"configuration.json").write_text(json.dumps(dict(
        ...
        robot_action_limit_seconds=args.simulation_time_limit_seconds,
        video=args.video,entrypoint="run_instruction_task.py",
        heading_navigation=args.heading_navigation,
        status="per_episode_record; study design is declared by the parent frozen protocol"),indent=2)+"\n")
    ...
    env=None; io=None; policy=None; video=None; move=None; action_clock=None; rows=[]; result={}
    ...
    try:
        ...
        (args.output/"perception_runtime.json").write_text(json.dumps(
            dict(owl_attention=perception.owl_attention_runtime,
                 owl_vision_precision=perception.owl_precision_runtime),indent=2)+"\n")
        ...
        limit=float(task.get("maximum_sim_seconds",1200))
        if args.simulation_time_limit_seconds is not None:limit=min(limit,args.simulation_time_limit_seconds)
        action_clock=RobotActionBudget(limit,base.control_freq)
        def before():
            action_clock.before_step(io.step_id)
        ...
        approached=False
        held=False
        result["pickup_candidate_attempts"]=[]
        for candidate_attempt in range(3):
            ...
            if not approached:break
            try:
                held=policy.attempt_grasp()
            except ValueError as error:
                if policy.grasp_motion_attempted or str(error)!="No valid observed grasp geometry after actual complementary head views":
                    raise
                policy.event("pickup_candidate_geometry_exhausted",reason=str(error))
                held=False
            result["pickup_candidate_attempts"].append(dict(index=candidate_attempt,held=held,
                grasp_motion_attempted=policy.grasp_motion_attempted))
            if held or not policy.defer_ungraspable_pickup():break
        result["grasp_proprioception_retained"]=held
        if held:
            policy.prepare_carry()
            policy.start_placement_search()
            result["placement_motion_completed"]=False
            for placement_attempt in range(3):
                if placement_attempt:
                    if not policy.defer_unusable_receptacle():break
                    policy.start_placement_search()
                placement_approach=policy.navigate(budget=args.navigation_budget)
                result["placement_approached"]=bool(result.get("placement_approached") or placement_approach)
                if not placement_approach:break
                result["placement_motion_completed"]=policy.place_observed()
                if result["placement_motion_completed"]:break
        ...
        if policy:
            policy.close_event_log()
            (args.output/"memory_updates.json").write_text(json.dumps(policy.memory.updates,indent=2)+"\n")
            if hasattr(policy.memory.observations, "stats"):
                (args.output/"observation_cache.json").write_text(
                    json.dumps(policy.memory.observations.stats(),indent=2)+"\n")
            if hasattr(policy.perception, "cached_detector"):
                (args.output/"perception_cache.json").write_text(
                    json.dumps(policy.perception.cached_detector.stats(),indent=2)+"\n")
        ...
        if action_clock is not None and io is not None:result.update(action_clock.report(io.step_id))
        result.update(wall_time_s=time.monotonic()-started,status="finished_instruction_attempt",
