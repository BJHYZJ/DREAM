...
def main():
    ...
    p.add_argument("--cohort-id", choices=["residential50_seed42_dynamic_v1"])
    ...
    if args.cohort_id:
        ...
        if (args.benchmark or len(task_data) != 50 or len({t["scene"] for t in task_data}) != 50
                ...
                or sorted(Counter(t["recipe"]["id"] for t in task_data).values()) != [10]*5
                or any(t["maximum_sim_seconds"] != 1800 for t in task_data)):
            raise ValueError("Residential cohort requires 50 distinct houses, seed 42, dynamic memory, 1800 seconds and 10 tasks per recipe")
