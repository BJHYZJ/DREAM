...
class CrossRoomSearchPilot(LearnedSearchPilot):
    ...
    def look_around(self):
        ...
        for offset in (0.,math.pi/2,math.pi,-math.pi/2):
            turned=self.turn_to(yaw+offset)
            if turned is False:
                relative=math.atan2(math.sin(yaw+offset-self.io.pose()[2]),math.cos(yaw+offset-self.io.pose()[2]))
                self.io.body[0]=float(np.clip(relative,-1.3,1.3))
                for _ in range(12):self.io.command()
            else:self.io.body[0]=0.
            result=self.observe()
    ...
    def geometry_observe(self):
        # Store high-frequency navigation depth separately from semantic views.
    ...
    def compact_navigation_arm(self):
        if self.io.grip < 0:
            return
        folded=self.io.fold_default_arm()
        self.event("navigation_arm_compacted",**folded,
    ...
    def move_chunk(self,path,max_cells=9,stop_on_detection=True,speed=.12,terminal_xy=None):
        ...
        if self.turn_to(math.atan2(*(chosen-base)[::-1])) is False:
            self.event("safety_replan_after_declined_turn")
            return False
        self.io.body[0]=0.
    ...
    def pick_frontier(self,start,distances,safe_override=None):
        ...
        if self.search_region_center is not None and self.regional_frontier_queries<getattr(self,"regional_frontier_limit",12):
    ...
    def try_heading_navigation(self,reason):
        return None  # Historical circular adapter has no heading fallback.
    ...
    def fallback_observation(self,moved,stalled,iteration):
        ...
        if stalled>=1 and self.exploration_goal is not None:
            goal=np.asarray(self.exploration_goal).copy()
            cell=self.occupancy.cells(goal)
            rr,cc=np.ogrid[:self.frontier_visits.shape[0],:self.frontier_visits.shape[1]]
            region=(rr-cell[0])**2+(cc-cell[1])**2<=(.48/self.occupancy.resolution)**2
            self.frontier_visits[region]=np.maximum(self.frontier_visits[region],3)
            self.exploration_goal=None
            self.heading_expansion_goal=None
            self.event("unproductive_frontier_deferred",point_xy=goal.tolist(),
                       scope="exploration_priority_only_obstacles_unchanged")
        return stalled+1

    def docking_limits(self):
        """Controller goal ring and visual handoff, not evaluator success limits."""
        return .58,.64,.68
        ...
        for iteration in range(budget):
            ...
            if result is not None:
                ...
                if result is not None and self.current_detection is not None:
                    ...
                    actual_range=np.linalg.norm(xyz[:2]-self.io.pose()[:2])
                    if actual_range<handoff:
                        self.event("visual_approach_complete",detection=asdict(self.current_detection),
                                   controller_handoff_distance_m=handoff,observed_range_m=float(actual_range))
                        self.planned_path=[]
                        self.waypoint=None
                        return True
            self.geometry_observe()
            ...
            if not safe[start]:
                ...
                moved=self.try_heading_navigation("circular_start_not_clear")
                if moved is not None:
                    stalled=self.fallback_observation(moved,stalled,iteration)
            ...
            expanded=self.try_exploration_expansion(start,distances,safe)
            if expanded is not None:
                stalled=self.fallback_observation(expanded,stalled,iteration)
                if stalled>=8:return False
                continue
            goal=None
            if self.cached is not None and self.exploration_goal is None:
                ...
                if ring.any():
                    ...
                    moved=self.try_heading_navigation("circular_docking_ring_unreachable")
                    if moved is not None:
                        stalled=self.fallback_observation(moved,stalled,iteration)
                        if stalled>=8:return False
                        continue
                    current=np.linalg.norm(self.cached[:2]-self.io.pose()[:2])
                    partial=(offsets>=dock_min)&(offsets<current-.18)
                    if partial.any():
            ...
            if exploring:
                # Preserve DREAM's existing reachable exploration choice in
                # open space. The heading adapter should recover a doorway or
                # a failed route, not replace every ordinary frontier decision.
                if self.search_anchor is not None and self.exploration_goal is None:
                    cells=np.argwhere(np.isfinite(distances)&safe)
                    ...
                    if len(cells):
                        ...
                        if offsets[best]<np.linalg.norm(self.io.pose()[:2]-self.search_anchor)-.12:
                            ...
                            # Circular inflation can seal the last-seen region
                            # even though the measured heading footprint fits.
                            # Keep this observed search prior available to the
                            # same guarded adapter before discarding it. It is
                            # NOT reinstated as a verified object/grasp target.
                            self.event("last_seen_anchor_circle_blocked",
                                point_xy=self.search_anchor.tolist(),
                                closest_observed_range_m=float(offsets[best]))
                            moved=self.try_heading_navigation("circular_last_seen_anchor_unreachable")
                            if moved is not None:
                                stalled=self.fallback_observation(moved,stalled,iteration)
                                if stalled>=8:return False
                                continue
            ...
            if goal is None:
                # Circular inflation can hide an accessible narrow passage.
                # Only now expand observed reachability with the measured
                # heading footprint; never use a task/house coordinate hint.
                moved=self.try_heading_navigation("observed_frontier_exploration")
                if moved is not None:
                    stalled=self.fallback_observation(moved,stalled,iteration)
                    if stalled>=8:return False
                    continue
            ...
            if not moved:
                # A circle-planned route can still fail after fresh depth or
                # an actual orientation check. Keep the same guarded fallback
                # available instead of repeatedly retrying that motion.
                recovered=self.try_heading_navigation("circular_route_execution_incomplete")
                if recovered is not None:
                    stalled=self.fallback_observation(recovered,stalled,iteration)
                    if stalled>=8:return False
                    continue
            if moved:
                stalled=0
