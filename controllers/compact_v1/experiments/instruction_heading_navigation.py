...
def candidate_route(planner,candidates,base,*,maximum_trials=32,source="memory_dock"):
    ...
    for cell,look_at in candidates:
        ...
        for yaw in headings:
            ...
            reachable=getattr(planner,"_reachable_pose_masks",None)
            if reachable is not None:
                r,c,h=planner.to_pt(goal)
                if not reachable[h,r,c]:continue
...
class InstructionHeadingNavigation:
    ...
    def try_exploration_expansion(self,start,distances,safe):
        ...
        if not getattr(self,"heading_navigation",False) or self.task_stage not in ("pickup_search","placement_search"):return None
        ...
        union=planner.reachable_pose_masks().any(axis=0)
    ...
    def try_heading_navigation(self,reason):
        ...
        planner=FetchHeadingAStar(self.occupancy,footprint,base,allow_reverse=False)
        safe=planner.reachable_pose_masks().any(axis=0)
        ...
        if self.cached is not None and self.exploration_goal is None:
            ...
            diverse=[];remaining=[]
            for cell in ordered:
                if all(np.linalg.norm((cell-other)*self.occupancy.resolution)>=.16 for other in diverse):
                    diverse.append(cell)
                else:remaining.append(cell)
            candidates=[(cell,self.cached[:2]) for cell in diverse+remaining]
            if not candidates:
