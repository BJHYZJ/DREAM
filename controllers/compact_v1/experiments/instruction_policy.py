        ...
        for recovery in (False,True):
            ...
            for step in range(int(round((12 if recovery else 2)*frequency))):
                ...
                if release is not None:
                    ...
                    positioned=(horizontal_error<min(clearance,.025)
                                and abs(center[2]-release[2])<.025)
                    if 0<clearance<.01 and speed<.02 and not positioned:
                        # Tight observed clearances expose a steady Cartesian
                        # servo error. Compensate through bounded commands;
                        # the original release center and fit checks stay fixed.
                        delta=.35*(release[:2]-center[:2])/frequency
                        delta*=min(1.,(.002/frequency)/max(np.linalg.norm(delta),1e-12))
                        alignment_bias+=delta
                        alignment_bias*=min(1.,.01/max(np.linalg.norm(alignment_bias),1e-12))
                ...
                if stable>=3:
                    ...
                    return True
        self.event("release_pose_unsettled",speed_m_s=speed,maximum_seconds=14.,
