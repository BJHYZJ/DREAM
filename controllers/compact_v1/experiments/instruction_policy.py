...
class InstructionSearchPilot(InstructionHeadingNavigation,CrossRoomSearchPilot):
    def __init__(self,io,perception,output,instruction,dynamic=True,heading_navigation=False):
        ...
        self.regional_frontier_limit=16
    ...
    def observe(self,sensor_name="fetch_head"):
        ...
        if (detection is None and self.task_stage in ("grasp","placement_search","place")
    ...
    def prepare_carry(self):
        self.require_retained_payload()
        # Retract from the pickup support before returning to the compact pose.
        # The same depth/proprioception-based sequence serves every object.
        ...
        self.io.robot.set_control_mode("pd_ee_delta_pose");self.io.robot.controller.reset()
        for reach in ((.55,) if self.perceived_payload_height>.12 else (.55,.40)):
        ...
        self.io.hold_measured_arm()
        self.phase="Fold arm for transport"
    ...
    def place_observed(self):
        ...
        self.phase="Withdraw above the placement surface"
        withdrawal=array(self.io.robot.tcp_pose.p)[0].copy()
        delta=withdrawal[:2]-self.io.pose()[:2]
        withdrawal[:2]-=delta/max(np.linalg.norm(delta),1e-9)*min(.25,max(0.,np.linalg.norm(delta)-.35))
        withdrawal[2]+=.05
        residual=self.io.move_tcp(withdrawal,target_rotation=vertical,tolerance=.015,steps=360)
        if residual>.03:raise RuntimeError("Released gripper could not retract above the table")
        clear=withdrawal+[0,0,.15]
        residual=self.io.move_tcp(clear,target_rotation=vertical,tolerance=.02,steps=360)
        if residual>.04:raise RuntimeError("Released gripper could not clear the placement surface")
        self.phase="Check released placement"
        for _ in range(60):
            self.io.command_ee()
        self.phase="Move clear of the table"
        self.io.hold_measured_arm()
        if getattr(self,"heading_navigation",False):
            base=self.io.pose().copy()
            goal=base[:2]-.18*np.array([math.cos(base[2]),math.sin(base[2])])
            if not self.heading_executor().translate(goal,planned_heading=base[2]):
                raise RuntimeError("No observed swept clearance to withdraw from the table")
        elif not self.retreat(.18):
            raise RuntimeError("No observed clearance to withdraw from the table")
        self.phase="Return arm to rest"
