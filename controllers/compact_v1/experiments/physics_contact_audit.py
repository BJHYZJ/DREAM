...
class ContactAudit:
    ...
    def after(self):
        ...
        for contact in self.base.scene.get_contacts():
            ...
            if a in self.entities and b in self.entities:
                force=sum(float(np.linalg.norm(point.impulse)) for point in contact.points)/float(self.base.scene.px.timestep)
                if force>=.5:
                    self.self_rows.append(dict(control_step=self.current_step,physics_substep=self.samples,
                        phase=self.phase,links=sorted([self.entities[a],self.entities[b]]),force_n=force))
                continue
    ...
    def report(self):
        return dict(physics_substeps=self.samples,physics_dt_s=float(self.base.scene.px.timestep),
            scope="Read-only contact sampling after every native PhysX substep during exact control re-execution. Includes robot/native and payload/native contacts. Robot self contacts are recorded separately; robot/target contacts and low horizontal floor support are excluded from environment contact counts. >=0.5 N contacts retained; fixtures distinguished from native environment.",
            includes_payload_native_contacts=True,
            robot_self_contact_rows=self.self_rows,
            robot_self_contact_control_steps=len({r["control_step"] for r in self.self_rows}),
