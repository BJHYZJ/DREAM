...
Task actor poses/meshes are never accepted. A held-object proxy uses observed
RGB-D dimensions and the recorded grasp frame at the measured TCP.
...
def observed_payload_vertices(tcp_matrix,radius,height,frame,margin=.02):
    """Bound an observed upright cylinder transported by a measured rigid grasp."""
    matrix=np.asarray(tcp_matrix,dtype=float)
    center=matrix[:3,3]+matrix[:3,:3]@np.asarray(frame["center_tcp_m"],dtype=float)
    axis=matrix[:3,:3]@np.asarray(frame["axis_tcp"],dtype=float)
    if not np.isfinite(center).all() or not np.isfinite(axis).all() or np.linalg.norm(axis)<1e-6:
        raise ValueError("Invalid observed payload frame")
    axis/=np.linalg.norm(axis)
    reference=np.eye(3)[int(np.argmin(np.abs(axis)))];first=np.cross(axis,reference);first/=np.linalg.norm(first)
    second=np.cross(axis,first);angles=np.arange(32)*2*np.pi/32
    ring=(radius+margin)/math.cos(math.pi/32)*(np.cos(angles)[:,None]*first+np.sin(angles)[:,None]*second)
    return np.vstack([center+sign*(height/2+margin)*axis+ring for sign in (-1,1)])


def capture_robot_footprint(io,payload_radius=None,padding=.025,payload_height=None,payload_frame=None):
    ...
    for link in io.robot.robot.get_links():
        for shape in link._objs[0].get_collision_shapes():
            ...
            pieces.append(xy)
            volumes.append((xy,float(world[:,2].min()),float(world[:,2].max())))
    ...
    if payload_radius is not None:
        ...
        if payload_frame is not None and payload_height is not None:
            world=observed_payload_vertices(as_array(io.robot.tcp_pose.to_transformation_matrix())[0],
                payload_radius,payload_height,payload_frame)
            disk=(world[:,:2]-base[:2])@world_to_base
            volumes.append((disk,float(world[:,2].min()),float(world[:,2].max())))
            payload=dict(observed_radius_m=float(payload_radius),observed_height_m=payload_height,
                frame=payload_frame,uncertainty_margin_m=.02,
                source="Observed support/object dimensions transported from the recorded grasp frame")
        else:
            ...
            volumes.append((disk,float(tcp[2]-bound),float(tcp[2]+bound)))
            payload=dict(observed_radius_m=float(payload_radius),observed_height_m=payload_height,
                conservative_projected_disk_radius_m=bound,tcp_base_xy=local.tolist(),
                source="Observed dimensions with no recorded grasp frame")
    ...
    layers=collision_height_layers(volumes)
    return dict(vertices_base_xy=boundary.tolist(),padding_m=float(padding),
        ...
        robot_links=links,payload_proxy=payload,height_layers=layers,
...
def collision_height_layers(volumes,edges=(-.25,.30,.60,.90,1.20,1.50,1.80,2.50)):
    """Bound each complete cooked collision shape in every slab it intersects."""
    if not volumes:return []
    lower=min(float(z0) for _,z0,_ in volumes)
    upper=max(float(z1) for _,_,z1 in volumes)
    cuts=sorted(set([min(lower,edges[0]),*edges[1:-1],max(upper,edges[-1])]))
    result=[]
    for bottom,top in zip(cuts,cuts[1:]):
        pieces=[xy for xy,z0,z1 in volumes if z1>=bottom and z0<=top]
        if not pieces:continue
        vertices=convex_boundary(np.vstack(pieces))
        result.append(dict(z_min_m=bottom,z_max_m=top,vertices_base_xy=vertices.tolist()))
    return result


def navigation_layer_views(occupancy,footprint):
    """Observed 3-D endpoints at overlapping heights, with a vertical margin."""
    layers=footprint.get("height_layers")
    if not layers:
        return [(occupancy.known,np.asarray(footprint["vertices_base_xy"]))]
    revision=occupancy.update_count
    cache=getattr(occupancy,"_height_layer_cache",None)
    if cache is None or cache[0]!=revision:
        cache=(revision,{})
        occupancy._height_layer_cache=cache
    margin=occupancy.resolution+footprint["padding_m"]
    views=[]
    for layer in layers:
        key=(layer["z_min_m"],layer["z_max_m"],margin)
        if key not in cache[1]:
            known=np.where(occupancy.known!=0,1,0).astype(np.int8)
            points=occupancy.occupied_xyz
            selected=(points[:,2]>=key[0]-margin)&(points[:,2]<=key[1]+margin)
            cells=occupancy.cells(points[selected,:2])
            cells=cells[occupancy.inside(cells)]
            known[cells[:,0],cells[:,1]]=-1
            cache[1][key]=known
        views.append((cache[1][key],np.asarray(layer["vertices_base_xy"])))
    return views
