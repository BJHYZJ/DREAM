...
class LearnedSearchPilot:
    ...
    def scan_sweep(self):
        ...
        self.event("head_camera_scan_complete",views=len(pans),sensor="fetch_head")
        return self.current_detection
    ...
    def attempt_grasp(self):
        ...
        self.perceived_payload_width=float(fit.get("closing_width_m",2*self.perceived_payload_radius))
        ...
        # Orient the open hand above the observed support before descending.
        def orientation_error():
            current=array(self.io.robot.tcp_pose.to_transformation_matrix())[0,:3,:3]
            return float(np.arccos(np.clip((np.trace(vertical@current.T)-1.)/2.,-1.,1.)))
        ...
        self.perceived_payload_frame=None
        ...
        return gap > max(.003,min(.015,.35*self.perceived_payload_width))
