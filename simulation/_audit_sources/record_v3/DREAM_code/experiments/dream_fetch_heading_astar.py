...
Edges are in-place rotations or forward translations. Robot collision geometry
...
from dream_fetch_footprint import orientation_masks,swept_mask,swept_clear
...
class FetchHeadingAStar(AStar):
    def __init__(self,occupancy,footprint,start_pose,max_expanded=250000):
        ...
        common=dict(known=occupancy.known,resolution=occupancy.resolution,vertices=self.vertices,
                    padding=self.padding,inferred=occupancy.inferred_blocked,offset=self.offset)
        self.pose_masks=orientation_masks(**common)
        self.forward_masks=[];self.turn_ccw_masks=[]
        for h,(dr,dc) in enumerate(self.steps):
            self.forward_masks.append(swept_mask(**common,start_yaw=h*math.pi/4,
                translation=np.array([dc,dr])*occupancy.resolution))
            self.turn_ccw_masks.append(swept_mask(**common,start_yaw=h*math.pi/4,angle=math.pi/4))
    ...
    def neighbor_points(self,state):
        ...
        return [(row,col,(heading+1)%8),(row,col,(heading-1)%8),(row+dr,col+dc,heading)]
    ...
    def edge_is_valid(self,start,end):
        ...
        if end!=(row+dr,col+dc,heading):return False
        if dr and dc and (self.occupancy.known[row+dr,col]!=1 or self.occupancy.known[row,col+dc]!=1):
            return False
        return bool(self.forward_masks[heading,row,col])
    ...
    def path(self,goal_pose):
        ...
        common=(self.occupancy.known,self.occupancy.origin,self.occupancy.resolution,self.vertices)
        if not swept_clear(*common,self.start_pose,snapped_start,padding=self.padding,
                           inferred=self.occupancy.inferred_blocked):return []
        try:
        ...
        if not swept_clear(*common,final,oriented,padding=self.padding,
                           inferred=self.occupancy.inferred_blocked):return []
    ...
    def validate_route(self,route):
        ...
        return all(swept_clear(self.occupancy.known,self.occupancy.origin,self.occupancy.resolution,
            self.vertices,a,b,padding=self.padding,inferred=self.occupancy.inferred_blocked)
