...
def memory_caption_present(captions,variant):
    text=[c["caption"] for c in captions]
    if variant=="static":
        return any("static memory kept" in c or "static memory retained" in c for c in text)
    return any("memory updated" in c for c in text)


def primary_endpoint_checks(report):
    """Keep shared proof; replace only the strict lost-and-found requirements.

    Never write this result into record_review_passed: a continuously tracked
    task cannot qualify as one of the requested loss-and-reacquisition videos.
    """
    strict_only={"protocol_scoring","reexecuted_protocol_success",
        "old_location_update_has_measured_free_depth","reacquisition_follows_observed_invalidation",
        "observed_memory_update_caption_visible"}
    checks={key:value for key,value in report["checks"].items() if key not in strict_only}
    checks.update(report["primary_task_evidence"])
    return checks


def verified_reassessment(run,physical_path,report_path):
    """Explicit score revision needs unchanged inputs AND matching fresh physics.

    A JSON success flag alone is insufficient. Retain the original verdict and
    bind the revised score to the exact evaluator used by independent replay.
    """
    report=json.loads(report_path.read_text())
    physical=json.loads(physical_path.read_text())
    replay_hashes=json.loads((physical_path.parent/"replay_source_hashes.json").read_text())
    original=json.loads((run/"result.json").read_text())
    required_inputs={"configuration.json","result.json","environment_task.json","events.jsonl","actions.json",
    ...
    valid=(report.get("status")=="offline_recorded_trajectory_reassessment_only"
        and Path(report["source_run"]).resolve()==run.resolve()
        and report.get("inputs_unchanged") is True and report.get("evaluator_unchanged") is True
        and required_inputs<=set(report.get("input_sha256",{}))
        and all(sha(run/name)==value for name,value in report["input_sha256"].items())
        and report["original_protocol_success"]==original.get("evaluator_protocol_success")
        and report["original_evaluation_version"]==original.get("evaluation_definition_version",1)
        and report["evaluation"]["evaluation_definition_version"]>report["original_evaluation_version"]
        and report["evaluation"]==physical["evaluation"]
        and report["evaluator_sha256"]==replay_hashes.get("experiments/instruction_evaluation.py")
        and physical.get("physical_reexecution_passed") is True
        and Path(physical["source_run"]).resolve()==run.resolve())
    if not valid:
        raise ValueError("Reassessment does not match unchanged source inputs and independent replay scoring")
    return report


def audit_record(run,physical,scoring_reassessment=None):
    ...
    revised=verified_reassessment(run,physical,scoring_reassessment) if scoring_reassessment else None
    scored_result=revised["evaluation"] if revised else result
    checks=dict(protocol_scoring=bool(scored_result.get("evaluator_protocol_success")),
    ...
    primary_evidence=dict(
        original_task_scoring=bool(scored_result.get("evaluator_task_success",scored_result.get("evaluator_protocol_success"))),
        reexecuted_task_success=bool(p["evaluation"].get("evaluator_task_success",p["evaluation"].get("evaluator_protocol_success"))),
        correct_visual_moved_target_observation=temporal["first_moved_detection_step"] is not None,
        all_logged_updates_have_observed_free_depth=all(r["measured_free_depth"] and r["time_aligned"] for r in updates))
    ...
    return dict(checks=checks,identity_checks=identity,memory_evidence=updates,
        temporal_evidence=temporal,primary_task_evidence=primary_evidence,
        memory_variant=config["variant"],review_definition_version=4,
        original_protocol_scoring=bool(result.get("evaluator_protocol_success")),
        scoring_reassessment=(dict(path=str(scoring_reassessment.resolve()),sha256=sha(scoring_reassessment),
            original_evaluation_version=revised["original_evaluation_version"],
            revised_evaluation_version=revised["evaluation"]["evaluation_definition_version"],
            criteria_changes=revised["criteria_changes"],independent_reexecution_score_matches=True)
...
def main():
    ...
    parser.add_argument("--scoring-reassessment",type=Path,
                        help="Explicit versioned offline score revision; must match unchanged inputs and independent physical replay")
    parser.add_argument("--output",type=Path,required=True)
    parser.add_argument("--annotate",action="store_true")
    parser.add_argument("--endpoint",choices=("strict","task"),default="strict",
                        help="Task endpoint permits continuous correct tracking; strict video qualification remains separate")
    args=parser.parse_args();args.output.mkdir(parents=True,exist_ok=False)
    report,result,task,events,rows=audit_record(args.run,args.physical_audit,args.scoring_reassessment)
    ...
    if args.annotate:
        variant_caption=memory_caption_present(caption_records,report["memory_variant"])
        report["checks"]["observed_memory_update_caption_visible"]=variant_caption
        report["checks"]["external_relocation_caption_visible"]=any(c["caption"].startswith("External:") for c in caption_records)
        report["primary_task_evidence"]["logged_updates_have_variant_correct_caption"]=not report["memory_evidence"] or variant_caption
    ...
    report["primary_task_checks"]=primary_endpoint_checks(report)
    report["primary_task_record_review_passed"]=all(report["primary_task_checks"].values())
    report["requested_endpoint"]=args.endpoint
    report["requested_endpoint_passed"]=(report["record_review_passed"] if args.endpoint=="strict"
                                       else report["primary_task_record_review_passed"])
    ...
    if not report["requested_endpoint_passed"]:raise SystemExit(1)
