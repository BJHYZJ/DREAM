...
def validate_docking_goal(report,query):
    """An explicit stand-off diagnostic changes no grasp/place success test."""
    bounds=np.asarray(report.get("docking_range_m",[.58,.64]),dtype=float)
    if (bounds.shape!=(2,) or not np.isfinite(bounds).all()
            or not 0<bounds[0]<=bounds[1]<=.85):
        raise ValueError("Invalid declared diagnostic docking range")
    distance=np.linalg.norm(np.asarray(report["route_xyyaw"][-1][:2])-np.asarray(query)[:2])
    if not bounds[0]-.04<=distance<=bounds[1]+.04:
        raise ValueError("Offline route does not terminate at its declared learned-query stand-off")
...
def run_diagnostic(io,source,output,prefix,route_path,target,destination,cart,step_budget=2400):
    ...
    validate_docking_goal(route_report,query)
    ...
    try:
        ...
        result=dict(navigation_motion_completed=completed,goal_error_m=goal_error,
            ...
            final_step=io.step_id,final_base_xyyaw=io.pose().tolist(),
