...
from dream.motion import ConfigurationSpace
from dream.motion import Node as BaseNode
from dream.motion import Planner, PlanResult
from dream.motion.algo.node import TreeNode as Node
...
class AStar(Planner):
    ...
    def neighbor_points(self, pt):
        """Default 2-D neighbors; embodiment adapters may supply motion states."""
        return neighbors(pt)
    ...
    def point_is_occupied(self, x: int, y: int) -> bool:
        ...
        """
        if x < 0 or y < 0 or x >= self._navigable.shape[0] or y >= self._navigable.shape[1]:
            return True
        return not bool(self._navigable[x][y])

    def edge_is_valid(self, start, end) -> bool:
        """Reject diagonal corner cutting on the footprint-inflated map."""
        if self.point_is_occupied(*end):
            return False
        if start[0] != end[0] and start[1] != end[1]:
            return not (self.point_is_occupied(start[0], end[1])
                        or self.point_is_occupied(end[0], start[1]))
        return True
    ...
    def is_in_line_of_sight(self, start_pt: Tuple[int, int], end_pt: Tuple[int, int]) -> bool:
        ...
        if start_pt == end_pt:
            return not self.point_is_occupied(*start_pt)
    ...
    def get_unoccupied_neighbor(self, pt: Tuple[int, int], goal_pt=None) -> Tuple[int, int]:
        ...
        neighbor_pts = self.neighbor_points(pt)
    ...
    def get_reachable_points(self, start_pt: Tuple[int, int]) -> Set[Tuple[int, int]]:
        ...
        while to_visit:
            ...
            for new_pt in self.neighbor_points(pt):
                if new_pt in reachable_points:
                    continue
                if not self.edge_is_valid(pt, new_pt):
                    continue
    ...
    def run_astar(
        ...
        while q:
            ...
            for nxt in self.neighbor_points(current):
                if not self.edge_is_valid(current, nxt):
                    continue
