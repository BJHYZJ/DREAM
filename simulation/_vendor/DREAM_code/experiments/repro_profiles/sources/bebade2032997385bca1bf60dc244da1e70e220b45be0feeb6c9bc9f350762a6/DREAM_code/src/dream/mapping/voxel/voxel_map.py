...
from dream.mapping.exploration_policy import select_frontier_goal
...
class SparseVoxelMapNavigationSpace:
    ...
    def sample_exploration(self, xyt, planner, text=None, semantic_rate:float=0.1, debug=False):
        ...
        decision = select_frontier_goal(
            ...
            min_distance_cells=min_dist_cells,
        )
        total_heuristics = decision.total_scores
        if decision.index is None:
            print("No valid exploration heuristic available.")
            return None
        index = np.asarray(decision.index)
        if debug:
            obstacles_np = (
