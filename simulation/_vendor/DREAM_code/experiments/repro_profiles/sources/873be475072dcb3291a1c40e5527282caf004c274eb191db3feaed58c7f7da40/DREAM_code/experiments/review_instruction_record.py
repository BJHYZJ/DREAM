...
def logged_captions(step,result,events,*,hz=20):
    ...
    if changes:
        event=changes[-1]
        captions.append(("Observed: memory updated" if event.get("dynamic_updates_enabled") else
                         "Observed: change; static memory retained",event["step"]))
...
def audit_record(run,physical):
    ...
    reacquired=result["reacquisition_step"]
    checks["reacquisition_follows_observed_invalidation"]=any(start<r["step"]<reacquired for r in updates)
    return dict(checks=checks,identity_checks=identity,memory_evidence=updates),result,task,events,rows
...
def main():
    ...
    report.update(decoded_frames=decoded,maximum_observation_panel_mae=max(errors),
        ...
        review_script_sha256=sha(__file__),release_ready=False,
