...
class InstructionSearchPilot(CrossRoomSearchPilot):
    ...
    def prepare_carry(self):
        self.require_retained_payload()
        if self.perceived_payload_radius<=.075:
            super().prepare_carry()
        else:
            # Long payloads can strike the robot during the small-object joint
            # fold. Preserve the measured grasp orientation, raise the torso
            # first, and retract through actual Cartesian servo segments.
            self.phase="Carry: preserve large-object grasp"
            ...
            self.event("measured_carry_envelope",robot=envelope,
                ...
                posture="orientation_preserving_large_payload")
            self.observe()
        self.require_retained_payload()
        return True
