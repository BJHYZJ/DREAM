...
def evaluate_instruction(rows,events,actions,move,room_map,*,hz=20):
    ...
    reacquired=[r for r in checks if r["evaluator_correct_identity"] and start is not None and r["step"]>start
        and np.linalg.norm(np.asarray(r["detection"]["point_world"][:2])-move.initial[:2])>.50]
    ...
    criteria=dict(instruction_before_initial_scan=bool(events and events[0]["event"]=="instruction_received"
        ...
        observed_stale_check=lost,correct_visual_reacquisition=bool(reacquired),
    ...
    return dict(evaluator_protocol_success=all(criteria.values()),criteria=criteria,
        ...
        reacquisition_step=reacquired[0]["step"] if reacquired else None,
