...
class InstructionReplayVideo:
    def __init__(self,source,review_path,output):
        self.source,self.output=source,output
        self.review=json.loads(review_path.read_text())
        if not self.review.get("record_review_passed") or self.review.get("review_definition_version")!=2:
            raise ValueError("Alternate spectator recording requires the stricter version-2 record audit")
    ...
    def frame(self,io,step,phase):
        ...
        self.records.append(dict(sim_step=step,phase=phase,eye_world=eye.tolist(),look_world=look.tolist(),
            fovy_rad=1.38,tcp_world=tcp.tolist(),captions=[c for c,_ in captions]))
