...
from dream.dynamic_memory import raycast_keep_mask
...
if not USE_TORCH_GEOMETRIC:
    from dream.utils.torch_geometric import consecutive_cluster, voxel_grid
    from dream.utils.torch_scatter import scatter
...
class VoxelizedPointcloud:
    ...
    def clear_points(
        ...
        if self._points is not None:
            indices = raycast_keep_mask(
                self._points.detach().cpu(),
                depth.detach().cpu(),
                intrinsics,
                camera_pose,
                min_distance_m=depth_in_view_min_distance,
                max_distance_m=depth_in_view_max_distance,
            )
    ...
    def add(
        ...
        obs_counts = torch.ones_like(weights) * obs_id
