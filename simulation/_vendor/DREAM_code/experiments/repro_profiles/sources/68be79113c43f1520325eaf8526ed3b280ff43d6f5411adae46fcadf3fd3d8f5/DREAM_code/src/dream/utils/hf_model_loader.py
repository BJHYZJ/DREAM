...
def build_hf_load_plan(repo_id: str, friendly_name: str) -> HFLoadPlan:
    # Optional immutable lock for reproducible offline simulation. The physical
    # robot's default cache discovery behavior is unchanged when unset.
    lock_file = os.environ.get("DREAM_MODEL_LOCK_FILE")
    if lock_file:
        import json

        revisions = json.loads(Path(lock_file).read_text())
        if repo_id not in revisions:
            raise ValueError(f"{friendly_name}: model not present in revision lock: {repo_id}")
        snapshot = hf_hub_root() / ("models--" + repo_id.replace("/", "--")) / "snapshots" / revisions[repo_id]
        if not is_valid_snapshot(snapshot):
            raise FileNotFoundError(f"{friendly_name}: locked model is not downloaded: {snapshot}")
        return HFLoadPlan(str(snapshot), True, str(snapshot))
