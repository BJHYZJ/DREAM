...
def freeze_sources(repo, frozen):
    """Copy exact source bytes and verify the complete tree before planning jobs.

    Hash the bytes read from the source, not an immediate read after a filesystem
    fast-copy. A previous NFS snapshot had one inconsistent recorded/file hash
    (underlying cause unconfirmed); all jobs stopped before simulation. Never repair a
    published manifest or continue past an inconsistent destination.
    """
    hashes={}
...
def main():
    ...
    protocol=dict(schema_version=1,mode="prespecified_60_attempt_paired_study" if args.benchmark else "development",
        ...
        requested_video_endpoint="evaluator_protocol_success (strict loss then reacquisition also required)",
