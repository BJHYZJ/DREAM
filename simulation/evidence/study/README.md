...
```bash
python -m simulation.verify_evidence
```
...
```bash
python simulation/_vendor/DREAM_code/experiments/run_instruction_profile.py \
  --study --gpus 0 --asset-dir simulation/.runtime/assets \
  --model-cache simulation/.runtime/models \
  --output simulation/results/my_comparison --execute
```
