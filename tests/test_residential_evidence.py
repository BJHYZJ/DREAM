...
def evidence(tmp_path):
    ...
    for i in range(50):
        ...
        with zipfile.ZipFile(path, "w") as archive:
            ...
            if i == 0:
                archive.writestr(
                    "audit/record/record_review.json",
                    json.dumps(dict(primary_task_record_review_passed=True)),
                )
        rows.append(
            dict(
                name=f"run{i}",
    ...
    report = dict(
        ...
        statistics=dict(variants=dict(dynamic=dict(attempts=50, successes=1, success_rate=0.02))),
    )
...
def test_residential_evidence_retains_every_failure(tmp_path):
    evidence(tmp_path)
    assert verify_residential(tmp_path) == dict(
        attempts=50, successes=1, compact_records_verified=True
    )
...
def test_residential_evidence_rejects_rehashed_inflated_summary(tmp_path):
    ...
    report["statistics"]["variants"]["dynamic"].update(successes=40, success_rate=0.8)
    path = tmp_path / "analysis/study_analysis.json"
    ...
    for i, row in enumerate(report["attempts"]):
        ...
        with zipfile.ZipFile(tmp_path / "attempts" / f"run{i}.zip", "a") as archive:
            archive.writestr(
                ...
                json.dumps(dict(recipe=dict(environment_assets=dict(pickup=row["pickup_model"])))),
            )
            if i == 0:
                archive.writestr(
                    "audit/physical/audit.json",
                    json.dumps(dict(contact_audit=dict(robot_self_contact_rows=[]))),
                )
