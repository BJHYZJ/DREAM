...
def glb(attributes):
    document=dict(meshes=[dict(primitives=[dict(attributes=attributes, material=0)])],
    ...
    return b'glTF'+struct.pack('<IIII',2,20+len(content)+len(binary),len(content),0x4E4F534A)+content+binary
...
def test_uv_alias_preserves_geometry_buffers_and_material_uv_selection():
    ...
    before=json.loads(original[20:20+before_length]);after=json.loads(output[20:20+after_length])
...
def test_derived_cache_preserves_upstream_and_binds_both_hashes(tmp_path):
    source=tmp_path/'original';path=source/'data/scene_datasets/lamp.glb';path.parent.mkdir(parents=True)
    payload=glb(dict(POSITION=0,TEXCOORD_1=2));path.write_bytes(payload)
    upstream=hashlib.sha256(payload).hexdigest()
    lock=tmp_path/'upstream.json';lock.write_text(json.dumps(dict(complete=True,failures=[],artifacts={'lamp.glb':dict(bytes=len(payload),sha256=upstream)})))
    output=tmp_path/'render';output_lock=tmp_path/'render.json'
    result=build_cache(source,output,lock,output_lock)
    assert result['changed_assets']==1 and path.read_bytes()==payload
    derived=output/'data/scene_datasets/lamp.glb';manifest=json.loads(output_lock.read_text())
    assert manifest['artifacts']['lamp.glb']['sha256']==hashlib.sha256(derived.read_bytes()).hexdigest()!=upstream
