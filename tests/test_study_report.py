...
def declared_batch(root: Path, success):
    jobs = [
        dict(
            name=f"test_attempt_{i:02d}",
            scene=f"test_house_{i // 6}",
            seed=i % 3,
            variant="dynamic" if i % 2 else "static",
            task="frozen_tasks/task.json",
        )
        for i in range(60)
    ]
    ...
    for job in jobs:
        job.update(
            task_sha256=sha(root / job["task"]),
            room_map_sha256=sha(root / "frozen_tasks/evaluator_room_map.npz"),
        )
    ...
    records = [
        dict(
            ...
            result_sha256=sha(first / "result.json"),
            source_verified_before=True,
            source_verified_after=True,
            task_verified_before=True,
            task_verified_after=True,
            status="completed",
        )
        for job in jobs
    ]
    write(
        root / "batch_result.json",
        dict(
            all_planned_attempts_recorded=True,
            attempts=records,
...
def batch_with_contact_rejection(tmp_path):
    ...
    for index, (job, record) in enumerate(zip(protocol["attempts"], batch["attempts"])):
        ...
        write(
            folder / "result.json",
            dict(evaluator_task_success=index == 0, source_files_unchanged=True),
        )
    ...
    physical = dict(
        ...
        evaluation={"evaluator_task_success": True},
        contact_audit={
    ...
    checks["no_native_contact_above_threshold"] = False
    review = dict(
        primary_task_checks=checks,
        primary_task_record_review_passed=False,
        physical_audit_sha256=sha(physical_path),
        events_sha256=sha(folder / "events.jsonl"),
    )
...
def test_contact_rejection_preserves_raw_outcome_and_full_denominator(tmp_path):
    ...
    assert len(report["attempts"]) == 60 and all(
        row["eligible_for_summary"] for row in report["attempts"]
    )
    ...
    assert report["audit_rejections"] == [
        dict(name="test_attempt_00", reason="native_contact_rejected", native_contact_steps=1)
    ]
...
def test_contact_reporting_never_swallows_other_audit_failures(tmp_path, failure):
    ...
    review["primary_task_checks"][failure] = False
    write(path, review)
    with pytest.raises(ValueError, match="primary task record audit did not pass"):
...
def test_contact_reporting_requires_the_complete_audit(tmp_path):
    root, audits, path = batch_with_contact_rejection(tmp_path)
    review = json.loads(path.read_text())
    del review["primary_task_checks"]["source_hashes_match"]
    write(path, review)
    with pytest.raises(ValueError, match="primary task record audit did not pass"):
        analyze(root, audits, tmp_path / "summary", include_contact_rejections=True)
