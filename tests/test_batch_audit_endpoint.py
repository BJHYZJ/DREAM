...
def test_audit_status_uses_requested_endpoint(tmp_path, monkeypatch, task_endpoint):
    ...
    (batch / "attempts.jsonl").write_text(
        json.dumps(
            dict(name="task", protocol_success=True, task_success=True, source_verified_after=True)
        )
        + "\n"
    )
    ...
    def run(command, **kwargs):
        ...
        if "replay_instruction_actions.py" in command[1]:
            ...
            (destination / "record_review.json").write_text(
                json.dumps(
                    dict(
                        record_review_passed=False,
                        primary_task_record_review_passed=True,
                        review_definition_version=4,
                    )
                )
            )
    ...
    monkeypatch.setattr(
        module.sys,
        "argv",
        ["audit", "--batch", str(batch), "--output", str(output)]
