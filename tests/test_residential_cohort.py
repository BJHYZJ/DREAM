...
def fixture(tmp_path):
    ...
    row = dict(id='01', scene='house-a', seed=42, variant='dynamic', file=task.name,
               sha256=sha(task.read_bytes()), room_map_sha256=sha(room.read_bytes()))
...
def test_residential_summary_keeps_all_failures_in_denominator():
    ...
    rows = [dict(scene=f'house-{i}', seed=42, variant='dynamic', task_success=i < 37) for i in range(50)]
    result = summarize_residential_outcomes(rows)
    assert result['variants']['dynamic'] == dict(attempts=50, successes=37, success_rate=.74)
...
def test_public_residential_tasks_use_fifty_houses_and_seed_42():
    ...
    assert sorted(Counter(json.loads(p.read_text())['recipe']['id'] for p in tasks).values()) == [10] * 5
