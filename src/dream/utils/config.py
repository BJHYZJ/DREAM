...
CONFIG_ROOT = str(Path(dream.__path__[0]).resolve() / "config")
CONTROL_CONFIG_DIR = str(Path(dream.__path__[0]).resolve() / "config" / "control")

DATA_ROOT = str(Path(dream.__path__[0]).parent.resolve() / "../data")
