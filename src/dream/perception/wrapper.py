...
from dream.perception.constants import RearrangeDETICCategories
...
class OvmmPerception:
    ...
    def __init__(
        ...
        if self._detection_module == "detic":
            # Lazy import
            from dream.perception.detection.detic import DeticPerception
            ...
            from dream.perception.detection.sam import SAMPerception

            self._segmentation = SAMPerception()

        elif self._detection_module == "mobile_sam":
            from dream.perception.detection.mobile_sam import MobileSAMPerception

            self._segmentation = MobileSAMPerception()
        elif self._detection_module == "sam2":
            from dream.perception.detection.sam2 import SAM2Perception

            self._segmentation = SAM2Perception()

        elif self._detection_module == "yolo":
            from dream.perception.detection.yolo import YoloPerception
            ...
            from dream.perception.detection.yolo_world import YoloWorldPerception
