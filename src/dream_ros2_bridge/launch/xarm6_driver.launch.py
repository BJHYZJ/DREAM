...
def launch_setup(context, *args, **kwargs):
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            'dof',
            default_value='6',
            description='Degree of freedom of manipulator, defalut is 6.',
        )
    )
    declared_arguments.append(
        DeclareLaunchArgument(
    ...
    robot_ip = LaunchConfiguration('robot_ip', default='192.168.1.233')
    ...
    robot_params = generate_robot_api_params(
        ...
        os.path.join(get_package_share_directory('dream_ros2_bridge'), 'config', 'xarm_user_params.yaml'),
