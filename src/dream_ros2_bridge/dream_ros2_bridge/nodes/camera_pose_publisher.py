...
from dream_ros2_bridge.constants import CAMERA_POSET_TOPIC


class CameraPosePublisher(Node):
    """Node that publishes camera pose transform [map -> camer_frame]"""

    def __init__(self, topic_name: str = CAMERA_POSET_TOPIC):
    ...
    def timer_callback(self):
        ...
        try:
            ...
            trans, rot = transform_to_list(
                self.tf_buffer.lookup_transform("map", "camera_color_optical_frame", rclpy.time.Time())
            )
