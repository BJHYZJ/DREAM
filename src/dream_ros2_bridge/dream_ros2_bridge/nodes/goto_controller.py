...
from dream.motion.control.goto_controller import GotoVelocityController
from dream.utils.config import get_control_config
from dream.utils.geometry import sophus2xyt, xyt2sophus
...
class GotoVelocityControllerNode(Node):
    ...
    def __init__(
        self,
        hz: float,
        # odom_only_feedback: bool = False,  # fast_lio2 odom + rtabmap is very accurate
        ...
        self.xyt: Optional[np.ndarray] = None
    ...
    def get_pose_from_tf(self):
        ...
        try:
            ...
            # Convert to sophus and then to xyt
            pose_sp = sp.SE3(matrix_from_pose_msg(pose_stamped.pose))
            self.xyt = sophus2xyt(pose_sp)
            
            # if not self.odom_only:
            #     self.controller.update_pose_feedback(self.xyt)
            self.controller.update_pose_feedback(self.xyt)
            ...
            self.get_logger().error(f'Error getting pose from TF: {e}')
            return False

    def _pose_update_callback(self, msg: PoseStamped):
        """Legacy method - now we get pose from TF instead"""
    ...
    def _odom_update_callback(self, msg: Odometry):
        ...
        # as ranger/odom include velocity, so we can use it to update the controller, linear.x is forward velocity, angular.z is yaw velocity, linear.y is not used
        self.vel_odom = np.array([msg.twist.twist.linear.x, msg.twist.twist.linear.y, msg.twist.twist.angular.z])
        # if self.odom_only:
    ...
    def _enable_service(self, request, response):
        ...
        self.get_logger().info("✅ Goto controller ENABLED and ready to receive goals")
        return response
    ...
    def _disable_service(self, request, response):
        ...
        self.get_logger().info("🛑 Goto controller DISABLED")
        return response
    ...
    def control_loop_callback(self):
        ...
        if self.active and self.xyt_goal is not None:
            ...
            if done and self.vel_odom is not None:
                if self.vel_odom[0] < VEL_THRESHOlD and self.vel_odom[2] < RVEL_THRESHOLD:
            ...
            if self.is_done:
                self.active = False
                self.xyt_goal = None
                self.get_logger().info('✅ The goal is done.')

    def create_pubs_subs_timers(self):
        """Publishers and Subscribers"""

        self.vel_command_pub = self.create_publisher(Twist, "/ranger/cmd_vel", 1)
        ...
        # )
        self.create_subscription(Odometry, "/ranger/odom", self._odom_update_callback, 1)  # wheel odom (ranger/odom) includ velocity, fast_lio2 odom only include position
