...
from dream_ros2_bridge.constants import T_LOC_STABILIZE
...
class DreamNavigationClient(AbstractControlModule):
    ...
    def get_base_in_map_pose(self, matrix=False) -> np.ndarray:
        """get the latest base pose from sensors"""
        if not matrix:
            return sophus2xyt(self._ros_client.get_base_in_map_pose())
        else:
            return self._ros_client.get_base_in_map_pose.matrix()
