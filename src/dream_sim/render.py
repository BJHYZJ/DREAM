...
from dream_sim.run import CATALOG, ENGINE, digest, execute, load_catalog, verify_source
...
def main() -> None:
    ...
    parser.add_argument(
        "--execution", type=Path, required=True, help="Root containing case_XX/execution"
    )
    parser.add_argument(
        "--audits",
        type=Path,
        help="Separate audit root containing case_XX/record; defaults to execution's own audits",
    )
    ...
    for name, target in (
        (".maniskill_assets", protocol["asset_root"]),
        (".dream_model_cache", protocol["model_cache"]),
    ):
    ...
    report = {
        ...
        "same_controls_spectator_rerender": True,
        "mode": "tool-front-placement",
    }
