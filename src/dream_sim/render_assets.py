...
def repair_uv_index(payload):
    ...
    for row in repairs:
        del restored['meshes'][row['mesh']]['primitives'][row['primitive']]['attributes']['TEXCOORD_0']
...
def build_cache(source, destination, reference_lock, output_lock):
    ...
    updated = copy.deepcopy(lock); changes = []
    for name, expected in lock['artifacts'].items():
        ...
        if repairs:
            ...
            changes.append(dict(file=name, source_sha256=expected['sha256'], rendered_sha256=digest(dst),
                binary_buffers_unchanged=True, repairs=repairs))
        else:
            try:
                os.link(src, dst)
            except OSError:
    ...
    updated['render_compatibility'] = dict(version=1, changes=changes,
    ...
    return dict(assets=len(updated['artifacts']), changed_assets=len(changes),
        output_lock_sha256=digest(output_lock), upstream_files_preserved=True)
...
def main():
    ...
    print(json.dumps(build_cache(args.source_dir, args.output_dir, args.reference_lock, args.output_lock), indent=2))
