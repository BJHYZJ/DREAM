...
def encode(source: Path, frames_path: Path, output: Path, *, crf: int = 20) -> dict:
    ...
    width, height = (
        int(capture.get(key)) for key in (cv2.CAP_PROP_FRAME_WIDTH, cv2.CAP_PROP_FRAME_HEIGHT)
    )
    fps = capture.get(cv2.CAP_PROP_FPS)
    if (width, height) != (1440, 600) or abs(fps - 5) > 1e-6 or count != len(frames):
        raise ValueError(
            "Expected the complete 1440x600, 5fps DREAM composite and matching frame record"
        )
    if any(row["frame"] != index for index, row in enumerate(frames)):
        raise ValueError("Frame record is not complete and ordered")
    if any(
        abs(frames[index]["sim_time_s"] - frames[index - 1]["sim_time_s"] - 1 / fps) > 1e-6
    ...
    command = [
        ...
        "+faststart",
        "-metadata",
        "comment=4x playback; all source frames preserved; overlay clock is simulation time",
        str(encoded_path),
    ]
    ...
    while True:
        ...
        if decoded in references:
            ...
            quality.append(
                {
                    "frame": decoded,
                    "full_ssim": ssim(reference, frame),
    ...
    passed = output.stat().st_size < 90 * 1024**2 and all(
        row["full_ssim"] >= 0.94 and row["head_ssim"] >= 0.94 and row["map_ssim"] >= 0.94
        for row in quality
    )
    report = {
        "source_video_sha256": source_hash,
        "frame_record_sha256": frames_hash,
        "output_sha256": digest(output),
        "output_bytes": output.stat().st_size,
        "source_frames": count,
        "decoded_output_frames": decoded,
        "resolution": [width, height],
        "source_fps": fps,
        "output_fps": out_fps,
        "playback_speed": 4,
        "source_duration_s": count / fps,
        "output_duration_s": count / out_fps,
        "codec": "h264",
